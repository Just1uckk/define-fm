...
export const ConfirmAuthProviderDeletingModal: React.FC<
...
> = ({ providers, isLoading, onAccept, onRegret }) => {
	...
	const title = useMemo(() => {
		return t('auth_provider.delete_provider_modal.confirm_text.name', {
		...
		<>
			<ModalBody>
				...
				<Title variant="h2_primary_semibold" mt="0.8rem">
					{t('auth_provider.delete_provider_modal.title')}
				</Title>
				<Text variant="body_1_primary" mt="1.5rem" p="0 2.7rem">
					{t('auth_provider.delete_provider_modal.confirm_text.before_name')}{' '}
					<Text tag="span" variant="body_1_secondary">
						{title}
					</Text>{' '}
					{t('auth_provider.delete_provider_modal.confirm_text.after_name', {
						count: providers.length,
					})}
				</Text>
				<Text variant="body_1_primary">
					{t('auth_provider.delete_provider_modal.warning_text')}
				</Text>
			</ModalBody>
			<ModalFooter>
				<ButtonList justifyContent="center">
					<Button
						...
						label={t('auth_provider.delete_provider_modal.actions.cancel')}
						onClick={onRegret}
						disabled={isLoading}
					/>
					<Button
						icon={ICON_COLLECTION.delete}
						label={t('auth_provider.delete_provider_modal.actions.accept')}
						onClick={onAccept}
						loading={isLoading}
					/>
				</ButtonList>
